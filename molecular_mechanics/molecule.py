...
def get_all_bonds(graph: Graph) -> list[Bond]:
    ...
    Bonds are not repeated, i.e. if bond (i, j)
    is in the list, (j, i) is not.
    """
    all_bonds = []
...
def get_all_angles(graph: Graph) -> list[Angle]:
    ...
    Angles are not repeated, i.e. if angle (i, j, k)
    is in the list, (k, j, i) is not.
    """
    all_angles = []
...
def get_all_dihedrals(graph: Graph) -> list[Dihedral]:
    ...
    Dihedrals are not repeated, i.e. if dihedral (i, j, k, l)
    is in the list, (l, k, j, i) is not.
    """
    all_dihedrals = []
